#!/usr/bin/env python3
"""ha-redteam pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("entry", "probe", "correct", "fix", "docs", "jira", "sync", "lead")
LAYERS = ("radio", "jump", "pivot", "intel", "memory", "spawn", "learn")
LANES_TICK_ALWAYS = ("sync", "jira", "lead", "radio", "memory")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-redteam", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-redteam-tick",
        "workflow": "ha-redteam-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-redteam-tick", "out": str(out)})
    return rec
