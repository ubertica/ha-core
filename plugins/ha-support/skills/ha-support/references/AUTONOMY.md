...
2. Run workflow `ha-support` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-support/scripts/ctl.sh forge \
  --name ha-support --prefix sup --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-support/scripts/ctl.sh selftest --name ha-support
bash ~/.grok/skills/ha-support/scripts/ctl.sh from-hackers   # dump checklist from gold
```
