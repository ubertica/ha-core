#!/usr/bin/env python3
"""ha-risk pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("rules", "cases", "score", "qa", "dev", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-risk", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-risk-tick", "out": str(out)})
    return rec
