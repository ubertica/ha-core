...
2. Run workflow `ha-risk` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-risk/scripts/ctl.sh forge \
  --name ha-risk --prefix rsk --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-risk/scripts/ctl.sh selftest --name ha-risk
bash ~/.grok/skills/ha-risk/scripts/ctl.sh from-hackers   # dump checklist from gold
```
