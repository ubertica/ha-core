#!/usr/bin/env python3
"""ha-payments pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("rails", "psp", "webhooks", "qa", "test", "dev", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-payments",
        "workflow": "ha-payments",
        "conductor": "ha-payments",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        ...
        "note": "Parent launches workflow. Skip READY lanes with on-disk artifacts. Tick always sync+lead.",
    }
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-payments", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-payments-tick",
        "workflow": "ha-payments-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-payments-tick", "out": str(out)})
    return rec
