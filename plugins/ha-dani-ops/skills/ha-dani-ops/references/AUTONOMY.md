...
3. Workflow `ha-dani-ops` (or `ha-dani-ops-tick`) — skip READY, spawn remainder, then lead
...
# then: workflow name=ha-dani-ops args.out=$OUT
```

Do not spawn `dops-*` by hand unless the workflow tool is down.
...
```
bash ~/.grok/skills/ha-dani-ops/scripts/ctl.sh watch --out "$OUT"
```

## Tests

```
bash ~/.grok/skills/ha-dani-ops/scripts/ctl.sh selftest
bash ~/.grok/skills/ha-dani-ops/scripts/ctl.sh verify --out "$OUT"
```
