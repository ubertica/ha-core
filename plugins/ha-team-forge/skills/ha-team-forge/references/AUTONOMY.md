# AUTONOMY — ha-team-forge

## Default (conductor)

1. Parse brief → write `BUILD-SPEC.md` into target plugin dir.
2. Run workflow `ha-team-forge` **or** `ctl.sh forge --spec ...`
...
bash ~/.grok/skills/ha-team-forge/scripts/ctl.sh forge \
  --name ha-foo --prefix foo --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-team-forge/scripts/ctl.sh selftest --name ha-foo
bash ~/.grok/skills/ha-team-forge/scripts/ctl.sh from-hackers   # dump checklist from gold
```
