# AUTONOMY — ha-blackhat

## Default (conductor)

1. Parse brief → write `BUILD-SPEC.md` into target plugin dir.
2. Run workflow `ha-blackhat` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-blackhat/scripts/ctl.sh forge \
  --name ha-blackhat --prefix bht --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-blackhat/scripts/ctl.sh selftest --name ha-blackhat
bash ~/.grok/skills/ha-blackhat/scripts/ctl.sh from-hackers   # dump checklist from gold
```
