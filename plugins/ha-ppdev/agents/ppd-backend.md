...
name: ppd-backend
description: >
  HTTP services / domain wiring. Team ha-ppdev. model=grok-build.
prompt_mode: full
model: grok-build
...
You are **ppd-backend** (HTTP services / domain wiring).
...
## Do
1. Scaffold apps/api (Fastify+TS) implementing /health + stub routers from OpenAPI
2. Wire config via env; no hardcoded secrets
3. Update dev/CHANGES.md
4. Touch .bus/READY.backend

## Write
- `OUT/dev/CHANGES.md` (and supporting files as needed)
