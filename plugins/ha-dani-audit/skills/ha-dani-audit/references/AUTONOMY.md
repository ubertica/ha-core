...
3. Workflow `ha-dani-audit` (or `ha-dani-audit-tick`) — skip READY, spawn remainder, then lead
...
# then: workflow name=ha-dani-audit args.out=$OUT
```

Do not spawn `daud-*` by hand unless the workflow tool is down.
...
```
bash ~/.grok/skills/ha-dani-audit/scripts/ctl.sh watch --out "$OUT"
```

## Tests

```
bash ~/.grok/skills/ha-dani-audit/scripts/ctl.sh selftest
bash ~/.grok/skills/ha-dani-audit/scripts/ctl.sh verify --out "$OUT"
```
