#!/usr/bin/env python3
"""ha-dani-audit pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("baseline", "mr", "release", "consistency", "secrets", "report", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-audit",
        "workflow": "ha-dani-audit",
        "conductor": "ha-dani-audit",
        "out": str(out),
        "ready": ready,
        "next": nxt,
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-audit", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-audit-tick",
        "workflow": "ha-dani-audit-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-audit-tick", "out": str(out)})
    return rec
