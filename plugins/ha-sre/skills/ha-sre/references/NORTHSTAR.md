...
Secondary gold: `ha-pumapay`, `ha-sentinel` (product + ops variants of the same shape). Cross: slo/deploy/oncall with sec/pam via bus.
...
Purpose: PumaPay SRE: SLOs, deploy, on-call. Complements ha-sentinel. Full HA autonomy, bus collab with sec/pam/ledger, selftest PASS. Same structure and quality as ha-hackers and ha-risk.
