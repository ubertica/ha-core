...
2. Run workflow `ha-fraud` **or** `ctl.sh forge --spec ...`
...
bash ~/.grok/skills/ha-fraud/scripts/ctl.sh forge \
  --name ha-fraud --prefix fraud --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-fraud/scripts/ctl.sh selftest --name ha-fraud
bash ~/.grok/skills/ha-fraud/scripts/ctl.sh from-hackers   # dump checklist from gold
```
