...
2. Run workflow `ha-sec` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-sec/scripts/ctl.sh forge \
  --name ha-sec --prefix sec --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-sec/scripts/ctl.sh selftest --name ha-sec
bash ~/.grok/skills/ha-sec/scripts/ctl.sh from-hackers   # dump checklist from gold
```
