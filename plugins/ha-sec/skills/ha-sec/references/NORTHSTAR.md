...
Secondary gold: `ha-pumapay`, `ha-sentinel` (product + ops variants of the same shape). Cross: threat/appsec with pam/sre via bus.
...
Purpose: PumaPay AppSec+SecOps (threat model, appsec, secops). Distinct from AMS sentinel. Full HA autonomy, bus collab with pam/sre/release/ledger, selftest PASS. Same structure and quality as ha-hackers and ha-risk.
