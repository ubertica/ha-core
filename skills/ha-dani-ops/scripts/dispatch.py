#!/usr/bin/env python3
"""ha-dani-ops pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("perimeter", "ams", "dash", "change", "escalate", "hold", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-ops",
        "workflow": "ha-dani-ops",
        "conductor": "ha-dani-ops",
        "out": str(out),
        "ready": ready,
        "next": nxt,
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-ops", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-ops-tick",
        "workflow": "ha-dani-ops-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-ops-tick", "out": str(out)})
    return rec
