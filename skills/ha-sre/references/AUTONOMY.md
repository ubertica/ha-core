...
2. Run workflow `ha-sre` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-sre/scripts/ctl.sh forge \
  --name ha-sre --prefix sre --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-sre/scripts/ctl.sh selftest --name ha-sre
bash ~/.grok/skills/ha-sre/scripts/ctl.sh from-hackers   # dump checklist from gold
```
