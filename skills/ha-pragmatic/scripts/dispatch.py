#!/usr/bin/env python3
"""ha-pragmatic pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("spec", "hash", "wallet", "games", "promo", "feeds", "bo", "test", "qa", "dev", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-pragmatic",
        "workflow": "ha-pragmatic",
        "conductor": "ha-pragmatic",
        "out": str(out),
        "ready": ready,
        "next": nxt,
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-pragmatic", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-pragmatic-tick",
        "workflow": "ha-pragmatic-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-pragmatic-tick", "out": str(out)})
    return rec
