# AUTONOMY — ha-pragmatic

## Default (conductor)

1. Parse brief → write `BUILD-SPEC.md` into target plugin dir.
2. Run workflow `ha-pragmatic` **or** `ctl.sh forge --spec ...`
...
bash ~/.grok/skills/ha-pragmatic/scripts/ctl.sh forge \
  --name ha-pragmatic --prefix pra --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-pragmatic/scripts/ctl.sh selftest --name ha-pragmatic
bash ~/.grok/skills/ha-pragmatic/scripts/ctl.sh from-hackers   # dump checklist from gold
```
