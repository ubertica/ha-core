#!/usr/bin/env python3
"""ha-dani-handoff pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("owners", "orphans", "bucket", "staffos", "docs", "sync", "lead")
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-handoff",
        "workflow": "ha-dani-handoff",
        "conductor": "ha-dani-handoff",
        "out": str(out),
        "ready": ready,
        "next": nxt,
    ...
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-handoff", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-dani-handoff-tick",
        "workflow": "ha-dani-handoff-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-dani-handoff-tick", "out": str(out)})
    return rec
