...
3. Workflow `ha-dani-handoff` (or `ha-dani-handoff-tick`) — skip READY, spawn remainder, then lead
...
# then: workflow name=ha-dani-handoff args.out=$OUT
```

Do not spawn `dhan-*` by hand unless the workflow tool is down.
...
```
bash ~/.grok/skills/ha-dani-handoff/scripts/ctl.sh watch --out "$OUT"
```

## Tests

```
bash ~/.grok/skills/ha-dani-handoff/scripts/ctl.sh selftest
bash ~/.grok/skills/ha-dani-handoff/scripts/ctl.sh verify --out "$OUT"
```
