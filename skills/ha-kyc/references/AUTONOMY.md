...
2. Run workflow `ha-kyc` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-kyc/scripts/ctl.sh forge \
  --name ha-kyc --prefix kyc --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-kyc/scripts/ctl.sh selftest --name ha-kyc
bash ~/.grok/skills/ha-kyc/scripts/ctl.sh from-hackers   # dump checklist from gold
```
