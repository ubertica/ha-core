#!/usr/bin/env python3
"""ha-blackhat pack router + remainder. Writes OUT/.bus/PLAN.json + NEXT.json. No grok spawn."""
...
LANES_FULL = ("entry", "probe", "exploit", "loot", "chain", "weapon", "docs", "lead")
LAYERS = ("radio", "jump", "pivot", "intel", "memory", "spawn", "learn")
LANES_TICK_ALWAYS = ("lead", "radio", "memory")

def ready_map(out: Path) -> dict[str, bool]:
    bus = out / ".bus"
    m = {}
    for lane in list(LANES_FULL) + list(LAYERS):
...
def plan(out_arg: str, lanes_json: str | None = None) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-blackhat",
        "workflow": "ha-blackhat",
        "conductor": "ha-blackhat",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        ...
        "note": "Core lanes required. Layers always-on via ctl.sh layers. Skip READY+artifact.",
    }
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-blackhat", "out": str(out)})
    return rec
...
def remainder(out_arg: str) -> dict:
    ...
    rec = {
        "ok": True,
        "pack": "ha-blackhat-tick",
        "workflow": "ha-blackhat-tick",
        "out": str(out),
        "ready": ready,
        "next": nxt,
        "action": "spawn" if nxt else "done",
    ...
    _write(out, "PLAN.json", rec)
    _write(out, "NEXT.json", {"next": nxt, "action": rec["action"], "pack": "ha-blackhat-tick", "out": str(out)})
    return rec
