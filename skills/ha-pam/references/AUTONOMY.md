...
2. Run workflow `ha-pam` **or** `ctl.sh forge --spec ...`
...
bash ~/.grok/skills/ha-pam/scripts/ctl.sh forge \
  --name ha-pam --prefix pam --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-pam/scripts/ctl.sh selftest --name ha-pam
bash ~/.grok/skills/ha-pam/scripts/ctl.sh from-hackers   # dump checklist from gold
```
