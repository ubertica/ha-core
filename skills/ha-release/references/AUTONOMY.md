...
2. Run workflow `ha-release` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-release/scripts/ctl.sh forge \
  --name ha-release --prefix rel --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-release/scripts/ctl.sh selftest --name ha-release
bash ~/.grok/skills/ha-release/scripts/ctl.sh from-hackers   # dump checklist from gold
```
