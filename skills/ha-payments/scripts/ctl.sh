#!/usr/bin/env bash
# ha-payments conductor CLI. Never nested grok -p.
set -euo pipefail
...
case "$cmd" in
  ...
  selftest)
    ...
    if [[ -f "$HOME/.grok/workflows/ha-payments.rhai" ]]; then
      echo "OK workflow: ha-payments.rhai"
    else
      echo "MISSING workflow ha-payments.rhai"; ERR=1
    fi
    if [[ -f "$HOME/.grok/workflows/ha-payments-tick.rhai" || "1" != "1" ]]; then
      echo "OK (or skipped) workflow: ha-payments-tick.rhai"
    else
      echo "MISSING workflow ha-payments-tick.rhai"; ERR=1
    fi
    if [[ -f "$HOME/.grok/skills/ha-payments/scripts/ctl.sh" || -L "$HOME/.grok/skills/ha-payments/scripts/ctl.sh" ]]; then
      echo "OK ~/.grok/skills/ha-payments/scripts/ctl.sh present"
    else
      echo "MISSING or no symlink in ~/.grok/skills/ha-payments/scripts/ctl.sh"; ERR=1
    fi
    if [[ $ERR -eq 0 ]]; then
      echo "SELftest PASS"
      exit 0
  ...
  auto)
    ...
    if [[ -z "$OUT" ]]; then
      OUT="${HA_PAYMENTS_OUT:-$HOME/Desktop/puma/docs/payments}"
    fi
...
" 2>/dev/null || echo ha-payments)
  ...
  tick)
    ...
    ;;
  status)
    OUT="${1:-${HA_PAYMENTS_OUT:-$HOME/Desktop/puma/docs/payments}}"
