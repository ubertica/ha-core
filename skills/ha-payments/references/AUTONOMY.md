# AUTONOMY — ha-payments

## Default (conductor)

1. Parse brief → write `BUILD-SPEC.md` into target plugin dir.
2. Run workflow `ha-payments` **or** `ctl.sh forge --spec ...`
...
```
bash ~/.grok/skills/ha-payments/scripts/ctl.sh forge \
  --name ha-payments --prefix pay --mode team \
  --purpose "..." --lanes lanes.json

bash ~/.grok/skills/ha-payments/scripts/ctl.sh selftest --name ha-payments
bash ~/.grok/skills/ha-payments/scripts/ctl.sh from-hackers   # dump checklist from gold
```
